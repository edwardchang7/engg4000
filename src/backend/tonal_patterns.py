            ...
            if mask and (temp_note != "" and temp_note != "^" and temp_note != "_"):
                ...
                try:
                    ...
                    if mask_check_bool:
                        ...
                        for_list, higher, lower, octave = add_to_dict_list(higher, lower, note, octave, degree_with_sharp, notes_in_key, temp_note, mask, notes_with_sharps)
                        if for_list != {}:
...
def add_to_dict_list(higher:bool, lower:bool, note:str, octave:int, degree_with_sharp:list, notes_in_key:list, temp_note:str, mask:bool, notes_with_sharps:list):
    ...
    if higher:
        if note=="'":
            ...
            if mask:
                return {"note": temp_note, "degree": str(notes_with_sharps.index(temp_note.upper()) + 1), "octave": octave}, False, lower, octave
            else:
                if degree_with_sharp[notes_in_key.index(temp_note.upper())]:
                    return {"note": temp_note + "#", "degree": str(notes_in_key.index(temp_note.upper()) + 1), "octave": octave}, False, lower, octave
                else:
                    return {"note": temp_note, "degree": str(notes_in_key.index(temp_note.upper()) + 1), "octave": octave}, False, lower, octave
    elif lower:
        if note==",":
            ...
            if mask:
                return {"note": temp_note, "degree": str(notes_with_sharps.index(temp_note.upper()) + 1), "octave": octave}, higher, False, octave
            else:
                if degree_with_sharp[notes_in_key.index(temp_note.upper())]:
                    return {"note": temp_note + "#", "degree": str(notes_in_key.index(temp_note.upper()) + 1), "octave": octave}, higher, False, octave
                else:
                    return {"note": temp_note, "degree": str(notes_in_key.index(temp_note.upper()) + 1), "octave": octave}, higher, False, octave
    else:
        return {}, higher, lower, octave
